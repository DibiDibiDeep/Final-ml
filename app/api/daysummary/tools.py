...
def cls_intent_assistant(query: str) -> str:
    ...
    Returns:
        str: 'QUESTION', 'DIARY_WRITE', 'DIARY_SAVE', 'SHARING'
    """
    llm = ChatOpenAI(
...
@tool
def sharing_assistant(query: str, chat_history: str) -> str:
    """
    When the user query intent is 'SHARING', this tool is used to generate empathetic responses.
    Parameters:
        query (str): The user's query
        chat_history (str): The chat history between the user and the assistant
    Returns:
...
def write_diary_assistant(day_information: str) -> str:
    ...
    Parameters:
        day_information (str): The information about the user and baby for the day
    Returns:
        str: The diary entry generated by the assistant
    """
    chain = (
        PromptTemplate.from_template(
        write_diary_template
    )
    # OpenAI의 LLM을 사용합니다.
    | ChatOpenAI(
        model=llm_model,
...
def save_diary_assistant(input_str: str) -> bool:
    ...
    Returns:
        bool: True if the diary entry is saved successfully, False otherwise
